...
import { MemberList } from "../../styles/pages/members";

export async function getStaticProps() {
  const reqMembersList = await fetch(
    `https://api.github.com/orgs/pizygroup/members`,
  );
  const membersList = await reqMembersList.json();
...
function Members({ membersList }: any) {
          ...
          {/* {membersList && membersData ? (
            <ul>
              {membersList
                .sort((a, b) => a.id - b.id)
                .map(user => {
                  const getUserData = membersData.filter(
                    member => member.id === user.login,
                  );
